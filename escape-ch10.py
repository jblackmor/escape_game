...
RECIPES = [ [62, 35, 63], [76, 28, 77], [78, 38, 54], [73, 74, 75], [59, 54, 60], [77, 55, 56], [56, 57, 58], [71, 65, 72], [88, 58, 89], [89, 60, 90], [67, 35, 68] ]
...
print('Recipes checksum:', checksum)
...
def use_object():
    ...
    if item_carrying == 70 or item_player_is_on == 70:
        ...
        time.sleep(0.5)
        # if player is "on" the computer, their intention is clearly to get a status update; return to stop another object use accidentally overriding this
        return
        ...
        energy = 100
        use_message = "You use the food to restore energy."
        remove_object(68)
        draw_energy_air()
   
    if suit_stitched and air_fixed:  # allows you to open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            props[20][0] = 0  # remove the door from the map
            sounds.say_airlock_open.play()
            show_text("The computer tells you the airlock is now open.", 1)
    ...
    for recipe in RECIPES:
        ...
        if (item_carrying == ingredient1 and item_player_is_on == ingredient2) or (item_carrying == ingredient2 and item_player_is_on == ingredient1):
            use_message = "You combine " + objects[ingredient1][3] + " and " + objects[ingredient2][3] + " to make " + objects[combination][3] + "."
            if item_player_is_on in props.keys():  # remove if a prop, but keep if scenery
...
def game_completion_sequence():
    global launch_frame  # initial value = 0, set up in the VARIABLES section
    box = Rect((0, 150), (800, 600))
